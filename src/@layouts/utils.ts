...
export const hiddenNavLink = () => {
  // Tìm thẻ a với href="/view-document"
  const link = document.querySelector<HTMLAnchorElement>('a[href="/view-document"]');
  const link2 = document.querySelector<HTMLAnchorElement>('a[href="/chinh-sua-tai-lieu"]');
  const link3 = document.querySelector<HTMLAnchorElement>('a[href="/view-image"]');

  if (link) {
    // Ẩn thẻ bằng cách đặt style display
