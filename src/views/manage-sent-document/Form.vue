...
import { DOCUMENT_TYPES, PAGE_URL, PIORTY, SECURITY, SentDocument, WORK_UNIT } from './contants';
...
const formData = ref<SentDocument>({ ...data })
...
<template>
  <VForm ref="refVForm" @submit.prevent="onSubmit">
    <VCard>
      <VCardItem>
        <VCardTitle>
          <span class="text-h4">{{ isEdit ? 'Văn bản đi' : 'Lấy số văn bản' }}</span>
        </VCardTitle>
      </VCardItem>
        ...
        <VCard>
          ...
          <VCardText>
            <VRow>
              ...
              <VCol cols="12" sm="6">
                <AppAutocomplete v-model="formData.workUnit" label="Đơn vị" placeholder="Chọn" required="required"
                  :items="WORK_UNIT" :rules="[requiredValidator]" />
              </VCol>
              <VCol cols="12" sm="6">
              ...
              <VCol cols="12" sm="6">
                <AppDateTimePicker v-model="formData.issueDate" label="Ngày ban hành" placeholder="Chọn ngày"
