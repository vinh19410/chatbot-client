...
import { API_URL, PartyPositionType } from './constants';

const initData: PartyPositionType = {
  title: "",
  agency: "",
  dateEnd: "",
  dateOfficial: "",
...
const page = ref(1)
const dataEdit = ref<PartyPositionType>(initData)
...
const headers: any[] = [
  ...
  { title: 'TÊN CHỨC VỤ ĐẢNG-ĐOÀN THỂ', align: 'left', key: 'title', sortable: false },
...
const total = computed(() => listData.value.totalItems)
...
<template>
  <section>
    <VCard class="mb-6">
      ...
      <!-- SECTION datatable -->
      <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items="list" :items-length="total"
        ...
        <VCard>
          ...
          <VCardItem>
            <VRow>
              ...
              <VCol cols="12">
                <AppTextField v-model="dataEdit.title" label="Tên chức vụ đảng-đoàn thể" required="required"
